...
public class AutoStepMoveForwardWithSensorSmart extends NKNAutoStep {
    AutoSkeleton autoSkeleton;
    private final double targ; private final double speed; private final double margin;

    public AutoStepMoveForwardWithSensorSmart(double target, double speed, double margin) {
        this.targ = target;
        this.speed = speed;
        this.margin = margin;
    }
...
//        autoSkeleton.relativeRun(0, adjSpeed);
    ...
    public boolean isDone(ElapsedTime runtime) {
        ...
        if (Math.abs(autoSkeleton.getSensorForDist()) < margin + targ) {
